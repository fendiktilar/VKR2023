...
class Background(Toplevel):
    ...
    def __init__(self, master, callback, graph_background_color='white', alpha='1.0'):
        ...
        ok = ttk.Button(self.btnFrame, text="Применить", command=self.send_data )
        ok.grid(row=0, column=0, ipady=7, ipadx=7)
        default = ttk.Button(self.btnFrame, text="Сохранить настройки", command=self.set_as_default)
